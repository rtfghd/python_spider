...
def get_taotu_links(taotu_url):
    ...
    for q in range(1,100):
        ...
        if '404' in is_exist:
            ...
            print('正在爬取性感美女板块套图链接，第%s页' % i, end='...')
            for title, link in zip(titles,links):
                ...
                f = open(r'E:/meizitu/links/xinggan_taotu.txt', 'a')
                f.write(data['link'] + '\n')
                f.close()
            print('完成')
            i += 1
    print('性感美女板块共有'+str(len(taotu_links))+'套图','\n')
    return taotu_links
    ...
    for j in range(1,100):
        ...
        try:
            ...
            if len(pictures_links) == 0: #判断图片链接列表里是不是为空
                ...
                f = open(r'E:/meizitu/links/xinggan_single.txt','a')
                f.write(link+'\n')
                f.close()
            else:
                if link in pictures_links:
                    ...
                    f.write(link+'\n')
                    f.close()
        except: #调试的时候发现有些套图的最后一页图片网页结构跟前面的不一样
            print('第%d套图的第%d张图片没有下载成功'%(k,j),'套图链接是这个，方便手动查找下载',picture_url)
            f = open(r'E:/meizitu/links/error.txt', 'a')
            f.write(picture_url + '\n')
            f.close()
            pass
...
if __name__ == '__main__':
    taotu_url = 'http://www.mzitu.com/xinggan/page'
