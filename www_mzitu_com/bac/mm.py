...
def get_taotu_links(taotu_url):
    ...
    for q in range(1,100):
        ...
        if '404' in is_exist:
            ...
            print('正在爬取清纯美女板块套图链接，第%s页' % i, end='...')
            for title, link in zip(titles,links):
                ...
                f = open(r'E:/meizitu/links/mm_taotu.txt', 'a')
                f.write(data['link'] + '\n')
                f.close()
            print('完成')
            i += 1
    print('清纯美女板块共有'+str(len(taotu_links))+'套图','\n')
    return taotu_links
    ...
    for j in range(1,100):
        ...
        try:
            ...
            if len(pictures_links) == 0: #判断图片链接列表里是不是为空
                ...
                f = open(r'E:/meizitu/links/mm_single.txt','a')
                f.write(link+'\n')
                f.close()
            else:
...
if __name__ == '__main__':
    taotu_url = 'http://www.mzitu.com/mm/page'
